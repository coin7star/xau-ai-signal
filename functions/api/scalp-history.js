...
export async function onRequest({ request, env }) {
  ...
  if (request.method === "POST") {
    ...
    const existing = await fbGet(dbUrl, `/xauusd/scalpHistory/${id}`);
    if (!existing) return json({ ok: false, error: "SCALP history tidak ditemukan" }, 404);
    ...
    await fbPut(dbUrl, `/xauusd/scalpHistory/${id}`, updated);
    ...
    return json({
      ok: true,
      message: `SCALP ${id} updated to ${result}`,
      item: updated,
      stats: buildStats(list)
    });
  }

...
export async function onRequest({ request, env }) {
  ...
  try {
    const firebaseLink = await createFirebasePasswordResetLink({ env, email, appUrl });
    const resetLink = buildDirectAuthActionLink({ firebaseLink, appUrl, env });

    const sent = await sendResendEmail({
      resendKey,
      from,
      to: email,
      subject: "Reset sandi XAU AI Signal",
      html: buildResetPasswordEmailHtml({ email, resetLink, appUrl }),
      text: `Reset sandi XAU AI Signal\n\nBuka link ini untuk membuat sandi baru:\n${resetLink}\n\nJika kamu tidak meminta reset sandi, abaikan email ini.`
    });

    if (!sent.ok) {
      return json({ ok: false, error: sent.error || "Gagal mengirim email reset." }, 502);
    }

    return json({ ok: true, message: "Email reset sandi berhasil dikirim." });
  } catch (error) {
...
async function createFirebasePasswordResetLink({ env, email, appUrl }) {
  ...
  if (!link) throw new Error("Firebase tidak mengembalikan reset link.");

  return link;
}

function buildDirectAuthActionLink({ firebaseLink, appUrl, env }) {
  try {

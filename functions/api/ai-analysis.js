...
async function callAI(env, market, signal) {
  ...
  const candlesM15 = (market?.candlesM15 || []).slice(-20);
...
TF Signal: ${market?.timeframe || "M1"}
TF Order Block: M15
Signal system: ${signal.signalLabel || signal.signal}
Call Stage: ${signal.callStage}
...
Bearish OB M15: ${JSON.stringify(signal.strategy?.orderBlock?.bearish || null)}
Recent M1 candles: ${JSON.stringify(candles)}
Recent M15 candles: ${JSON.stringify(candlesM15)}
